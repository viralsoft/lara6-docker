...
@section('content')
    <section class="content-header">
        <h1>
            ...
            <small>{{ __('virals-inventory::labels.unit_update') }}</small>
        </h1>
        <ol class="breadcrumb">
            <li><a href="/home"><i class="fa fa-home"></i>{{ __('virals-inventory::messages.home') }}</a></li>
            <li class="active">{{ __('virals-inventory::labels.unit_update') }}</li>
        </ol>
    </section>
            ...
            <div class="box-body">
                <div class="box">
                    <div class="box-header">
                        <h3 class="box-title">{{ __('virals-inventory::labels.unit_update') }}</h3>
                    </div>
                </div>
                @if ($errors->any())
                ...
                @endif
                {!! Form::open(['url' =>[route('admin.units.update', $unit->id) ] , 'method'=> 'POST','files' => true]) !!}
                @method('PUT')
                <input type="hidden" name="id" value="{{ $unit->id }}">
                <div class="form-group">
                    {!! Form::label('name', __('virals-inventory::labels.unit_name')) !!}
                    {!! Form::text('name', old('name') ?? $unit->name , ['class' => 'form-control', 'placeholder' => __('virals-inventory::labels.unit_name')]) !!}
                </div>
